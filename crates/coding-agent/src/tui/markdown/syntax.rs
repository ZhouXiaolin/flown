...
pub(super) fn highlight_code(
    ...
    let max_inner_width = render_width.saturating_sub(4).max(display_width(label) + 3);
    let inner_width = if full_width {
        max_inner_width
    } else {
        let max_text = raw.iter().map(|(_, w)| *w).max().unwrap_or(0);
        let min_inner = (display_width(label) + 3).max(44).min(max_inner_width);
        (max_text + 2).max(min_inner).min(max_inner_width)
    };
