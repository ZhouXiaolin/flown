...
pub fn render_entry(entry: &ConversationEntry) -> Vec<Line<'static>> {
    match &entry.kind {
        ...
        EntryKind::Assistant(text) => render_markdown("● ", Color::Rgb(118, 205, 255), text),
        ...
        EntryKind::System(text) => render_markdown("ℹ ", Color::DarkGray, text),
    }
}
...
fn render_markdown(prefix: &'static str, color: Color, body: &str) -> Vec<Line<'static>> {
    ...
    let (mut lines, _, _) = parse_markdown(body, ss, theme, &app_theme.markdown, false);
    if lines.is_empty() {
